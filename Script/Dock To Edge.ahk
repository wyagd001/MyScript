...
If Edge_Dock_Position_%Gui_wid% contains Left
    Edge_Dock_X := - ( Edge_Dock_Width_%Gui_wid% - Edge_Dock_Border_Visible ) + MWAleft
Else If Edge_Dock_Position_%Gui_wid% contains Right
    Edge_Dock_X := MWAright - Edge_Dock_Border_Visible
If Edge_Dock_Position_%Gui_wid% contains Top
    Edge_Dock_Y := - ( Edge_Dock_Height_%Gui_wid% - Edge_Dock_Border_Visible )+ MWAtop
Else If Edge_Dock_Position_%Gui_wid% contains Bottom
    Edge_Dock_Y :=MWAbottom - Edge_Dock_Border_Visible
...
;tooltip % MWAtop
    ...
    Else If Edge_Dock_Position_%Edge_Dock_Active_Window% contains Right
      Edge_Dock_X := MWAright - Edge_Dock_Width_%Edge_Dock_Active_Window%
    ...
    Else If Edge_Dock_Position_%Edge_Dock_Active_Window% contains Bottom
      Edge_Dock_Y := MWAbottom - Edge_Dock_Height_%Edge_Dock_Active_Window%
...
Slide(Win,X,Y,Dir,Num=10, Sleep=5)  ;Thanks Infogulch for the idea behind this.
{
	...
	else if(dir="In")
	{
		...
		{
			if(x<0 and x <> "")
				WinMove_X:=(-A_Index) * Dock_Width / Num + MWAleft
			if(x>0)
				WinMove_X:=MWAright+ A_Index * Dock_Width / Num - Dock_Width
			if(y<0 and y <> "")
				WinMove_Y:=(-A_Index ) * Dock_Hight / Num + MWAtop
			if(y>0)
				WinMove_Y:=MWAbottom + A_Index * Dock_Hight / Num - Dock_Hight
			WinMove, ahk_id %Win%,, %WinMove_X%, %WinMove_Y%
			Sleep, %Sleep%
		}
	}
