...
Label_Candy_Read_Value:
	...
	{
		Goto Label_Candy_RunCommand            ; 如果不是menu指令，直接跳转到运行应用程序
	}
...
Label_Candy_DrawMenu:
	...
	MouseMove, CandyMenu_X, CandyMenu_Y, 0
	;ToolTip, % A_TickCount-CandyStartTick, 200, 0     ; 若要评估弹出menu时间，这里需打开 ,共三处，2/3
    Menu, CandyTopLevelMenu, show
	;ToolTip ; 若要评估弹出menu时间，这里需打开 ,共三处，3/3
Return
...
*/
	...
	{
		CandySelected := CandySel    ; 兼容以前的cando变量写法
		If IsStringFunc(Candy_Cmd_Str2)
		{
			RunStringFunc(Candy_Cmd_Str2)
		return
		}
		Else
			Goto Label_Candy_ErrorHandle
