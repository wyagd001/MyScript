...
Label_Windy_Set_Pos_or_WinArea:
   ;Gosub Label_Windy_The_Position        ;划分固定的位置区
    ...
    If ( Windy_The_Position!="")  ;如果是固定区域
    {
        ...
        {
            IfExist,%Windy_Profile_Ini_Dir%\位置区\%Windy_The_Position%.ini
            {
...
;Windy_Cmd:=SkSub_Ini_RegexFindValue(Windy_Profile_Ini, Windy_Window_Area , "i)(^|\|)\Q" Windy_CurWin_ProcName "\E($|\|)")
            If(Windy_Cmd="Error") ;如果没有定义，则看看AnyWindow的定义
            {
                ...
                If(Windy_Cmd="Error") ;如果没有定义，则看看AnyWindow的定义
                {
                    ...
                    IfExist,%Windy_Profile_Ini_Dir%\%Windy_Window_Area%\%Windy_CurWin_ProcName%.ini
                    {
                        Windy_Cmd:="menu|" Windy_CurWin_ProcName "|menu"
                    }
                    Else IfExist,%Windy_Profile_Ini_Dir%\%Windy_Window_Area%\Any.ini
                    {
...
Label_Windy_DrawMenu:
    ...
    WindyMenu_sec:= WindyMenuFrom3="" ? "Menu" : WindyMenuFrom3
     ...
     szMenuContent:={}
     szMenuWhichFile:={} 
    _GetMenuItem(Windy_Profile_Ini_Dir "\" Windy_Window_Area,WindyMenu_ini,WindyMenu_sec,"Windymenubywannianshuyao","")
...
;[············································································································]
   ...
   ; }
    If(RegExMatch(Windy_Cmd,"i)^(Windo\|)")) ;如果是以windo|开头，则是运行内置标签
    {
        ...
        If IsLabel( Windy_WindoLabel_str)                       ;程序内置的别名
    ...
    Else If (RegExMatch(Windy_Cmd,"i)^SetClipBoard\|"))   ;纯粹的置粘贴板动作，这个与前面的{setclipboard:pure}{setclipboard:rich}不一样，这个是指令
    {
    ...
    Else If(RegExMatch(Windy_Cmd,"i)^(mute\|)")) ;如果是以Msgbox|开头，则是发一个消息框
    {
        Send {Volume_Mute}
    }
