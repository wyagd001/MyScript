...
, Start := False
, AutoAHK := False
, AutoAHKPath := A_ScriptDir . "\..\AutoAHK.ahk"
, AutoAHKData
, Includes
, hEdit
, VScrollBarW
...
, IconLib := A_ScriptDir . "\..\pic\AutoAHK.icl"
, FocusText := False
...
If (FileExist(A_AppData . "\AutoAHK\Find in Files.ini")) {
    IniFile := A_AppData . "\AutoAHK\Find in Files.ini"
} Else {
...
If (FileExist(IniFile)) {
    ...
    Gui Show, y100 w794 h484 Hide, 在文件中查找
}

GetClientSize(hMainWnd, GuiWidth, GuiHeight)

w := GuiWidth - 8 - 6 - 84 - 6
Gui Add, Tab3, hWndhTab x8 y8 w%w% h220, 搜索

cw := GuiWidth - 111 - 8 - 80 - 14 - 6 - 84 - 6
Gui Add, Text, x22 y47 w86 h23 +0x200, 目录:
Gui Add, ComboBox, hWndhCbxDir vDirectory x111 y47 w%cw%, %DirHistory%

x := GuiWidth - 12 - 84 - 80 - 14
If (NT6orLater) {
    Gui Add, Button, hWndhBtnBrowse gBrowse x%x% y46 w80 h24, 浏览(&B)
} Else {
...
Gui Add, Text, x22 y88 w86 h23 +0x200, 过滤:
...
Gui Add, Text, x22 y130 w86 h23 +0x200, 查找的:
Gui Add, ComboBox, hWndhCbxFind vTextToFind x111 y130 w%cw%, %FindHistory%

Gui Add, Text, hWndhSep2 x22 y162 w%sw% h2 +0x10

Gui Add, CheckBox, vChkRecurse x22 y171 w160 h23 +Checked, 搜索子目录(&S)
Gui Add, CheckBox, vChkSingleMatch x22 y194 w160 h23 +Checked%SingleMatch%, 每文件匹配一次(&O)
Gui Add, CheckBox, vChkMatchCase gSetExclusivity x190 y171 w160 h23, 区分大小写(&C)
Gui Add, CheckBox, vChkWholeWords gSetExclusivity x190 y194 w160 h23, 匹配整词(&M)
Gui Add, CheckBox, vChkRegEx gSetExclusivity x358 y171 w160 h23, 正则表达式(&R)
Gui Add, CheckBox, vChkBackslashes gSetExclusivity x358 y194 w160 h23, 转换反斜杠(&b)
Gui Add, CheckBox, vChkNotContaining x526 y171 w160 h23, 不包含文本(&N)
Gui Add, CheckBox, vChkHexSearch gSetExclusivity x526 y194 w160 h23, 十六进制搜索(&H)

Gui Tab

x := GuiWidth - 84 - 6
Gui Add, Button, hWndhBtnStart gStartSearch x%x% y7 w84 h24 +Default, 开始
Gui Add, Button, hWndhBtnCancel gCancelSearch x%x% y38 w84 h24, 取消
Gui Add, Button, hWndhBtnHelp gShowHelp x%x% y69 w84 h24, 帮助
...
If (AutoAHK) {
    Menu SplitButtonMenu, Add, 当前目录, MenuHandler
    Menu SplitButtonMenu, Add, 当前文件, MenuHandler
    Menu SplitButtonMenu, Add, 当前文件和它包含的, MenuHandler
    Menu SplitButtonMenu, Add, 所有打开的文件, MenuHandler
    If (NT6orLater) {
...
StartSearch:
    ...
    If (Directory == "") {
        ...
        Edit_ShowBalloonTip(hCbxEdit, "目录不能为空")
        Return
    }
    ...
    For Each, Dir in Directories {
        ...
        Loop Files, %Dir%%LoopMask%, % ChkRecurse ? "R" : ""
        {
            ...
            SB_SetText(" 搜索中……: " . A_LoopFileFullPath)
    ...
    SB_Text := Stop ? " 搜索中止. " : " 搜索结束. "
    If (TextToFind != "") {
        Files := FileCount > 1 ? "文件" : "文件"
        If (MatchCount) {
            Matches := MatchCount > 1 ? "匹配" : "匹配"
            SB_Text .= MatchCount . " " . Matches . " 在 " . FileCount . " " . Files . "."
        } Else {
            If (ChkNotContaining && FileCount) {
                SB_Text .= "未找到在 " . FileCount . " " . Files . "."
            } Else {
                SB_Text .= "未找到."
            }
        }
...
SaveSettings() {
    If (!FileExist(IniFile)) {
        ...
        If (ErrorLevel) {
            FileCreateDir %A_AppData%\AutoAHK
            IniFile := A_AppData . "\AutoAHK\Find in Files.ini"
            FileDelete %IniFile%
            FileAppend %Sections%, %IniFile%, UTF-16
        }
...
;msgbox % Haystack " - " arrNeedle.Length() " - " arrNeedle[A_Index]
...
OpenWithAutoAHK() {
    SendMessage 10000, 1, %hEdit%,, % "ahk_id " . GetAutoAHKHandle(AutoAHKPath)
}
...
MenuHandler:
    ; AutoAHK message: request list of open files
    SendMessage 10000, 2, %hMainWnd%,, % "ahk_id " . GetAutoAHKHandle(AutoAHKPath)
    ...
    CurrentFile := StrSplit(AutoAHKData, ";")[1]
    ...
    If (Item == 1) { ; Current directory
        ...
        GuiControl Text, Directory, %AutoAHKData%
    }
Return
...
GetAutoAHKHandle(AutoAHKPath) 
{
    If (!hWnd := WinExist("AutoAHK v")) 
    {
        Try 
        {
            Run %AutoAHKPath%,,, AutoAHKPID
        } 
        Catch e 
        {
        ...
        WinWaitActive ahk_pid %AutoAHKPID%,, 3
        If (ErrorLevel) 
        {
            MsgBox 0x15, Error, 窗口激活超时. 重试吗?
            IfMsgBox Retry
            {
                GetAutoAHKHandle(AutoAHKPath)
            }
        ...
        Else 
        {
            WinGet hWnd, ID, ahk_pid %AutoAHKPID%
        }
    }
    Return hWnd
}
...
CustomMessage(wParam,lParam) 
{
    MsgBox,%lParam%
    If (wParam == 2) 
    { ; Message sent by AutoAHK
        ControlGetText AutoAHKData,, ahk_id %lParam%
