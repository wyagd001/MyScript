...
Global AppName := "AutoHotkey Scripts Manager"
     ...
     , g_pIContextMenu2
     , g_pIContextMenu3
     , IconLib := A_ScriptDir "\..\pic\Scripts Manager.icl"
...
Menu ScriptsListMenu, Add, RunningList, LoadList
Menu ScriptsListMenu, Add, MangerList, LoadList
Menu ScriptsListMenu, Add, FavList, LoadList
...
LoadList(ListName:="") {
  if (ListName = "") || (ListName = "RunningList")
  {
    h_list := "RunningList"
  ...
  else if (ListName = "FavList")
  {
    ...
    Tmp_Str := StrReplace(Tmp_Str, "=1")
    Loop, Parse, Tmp_Str, `n, `r
    {
      ahk_loop := A_LoopField
      SplitPath, ahk_loop, name
      LV_Add("", name, ahk_loop,"","")
    }
    ReloadList()
  }
  else
  {
    ...
    Loop, %A_ScriptDir%\..\脚本管理器\*.ahk
    {
      ...
      ifinstring, A_LoopFileName, !
        ...
        LV_Add("Check", A_LoopFileName, A_LoopFileLongPath, "", "")
    }
    ReloadList()
    LoadLV_dis_Label := 0
  }
...
GuiContextMenu:
If (A_GuiControl == "MyList" && Row := LV_GetNext()) {
...
if LoadLV_dis_Label
return
If (A_GuiEvent = "DoubleClick") or (A_GuiEvent = "A")
{
  LV_GetText(hpid, LV_GetNext(), 3)
  LV_GetText(hfile, LV_GetNext(), 2)
  SplitPath, hfile, , hDir
  if !hpid
    run %hfile%, %hDir%
  return
}
else If (A_GuiEvent == "I") {
  if (h_list = "MangerList")
  {
    If (ErrorLevel == "C")
    {
      LV_GetText(hfile,A_EventInfo,2)
      SplitPath, hfile, hname, hDir
      FileMove, %hfile%, % hDir "\" LTrim(hname, " !")
      LoadList("MangerList")
    return
    }
...
{
	IniWrite, 1, %IniFile%, Fav, %A_LoopField%
	LoadList("Favlist")
}
return
...
SaveSettings() {
    If (!FileExist(IniFile)) {
        ...
        If (ErrorLevel) {
            ...
            IniFile := A_AppData . "\AutoAHK\Scripts Manager.ini"
            FileDelete %IniFile%
            FileAppend %Sections%, %IniFile%, UTF-16
        }
    }
